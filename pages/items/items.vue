<template>
	<view class="content">
		...
		<select-box @showDownlist="showDownlist" :listData="selectValue" :showSelect="showSelect"></select-box>
		<downlist-vertical ref="downfilter" :listData="currentDownList" :value="currentDownListValue" @seclecItem="seclecItem"
		 @formatArrow='fArrow'></downlist-vertical>
		<tree ref="tree" @choice="choiceKnowList" @hideSelect="hideSelect" :treeDatab="treeData"></tree>
		<item-list ref="itemlist" :dataList="listData" rurl="Commonajax/searchQtrunk" :rdata="rdata" :paddTop="paddTop"></item-list>
	</view>
</template>
...
<script>
	export default {
		...
		onShow() {
			...
			this.initData();
			this.getDonwList();
			const open = getApp().globalData.itemsOpen;
			if(open){
				this.showTree();
			}
		},
